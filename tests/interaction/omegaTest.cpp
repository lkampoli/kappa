...
int main(int argc, char** argv) {
  ...
  for (int i=0; i<molecules.size(); i++) {
    for (int k=i; k<molecules.size(); k++) {
      ...
      for (auto T : T_vals) {
        out_m1_m2 << std::setw(20) << T << std::setw(20) << int_m1_m2["collision diameter"] << std::setw(20);
        for (auto model : omega_integral_models) {
  ...
  for (int i=0; i<molecules.size(); i++) {
    for (int k=0; k<atoms.size(); k++) {
      ...
      for (auto T : T_vals) {
        out_m1_m2 << T << std::setw(20) << int_m1_a1["collision diameter"] << std::setw(20);
        for (auto model : omega_integral_models) {
  ...
  for (int i=0; i<atoms.size(); i++) {
    for (int k=i; k<atoms.size(); k++) {
	 ...
	 out_m1_m2 << std::setw(15) << curr_model << "(" << to_string(l) + "," + to_string(r) + ")" << std::setw(20);
       ...
       out_m1_m2 << endl;
       for (auto T : T_vals) {
         out_m1_m2 << T << std::setw(20) << int_a1_a2["collision diameter"] << std::setw(20);
	 for (auto model : omega_integral_models) {
	   try {
