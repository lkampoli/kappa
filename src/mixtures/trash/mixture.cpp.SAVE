...
// end of Mixture constructors ////////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  void kappa::Mixture::init_matrices(const std::string &particles_filename) {
    ...
    // diffusion_RHS.zeros((n_vibr_levels_total + num_atoms) * (n_vibr_levels_total + num_atoms));
    // diffusion_rigid_rot_RHS();
    diffusion_RHS.zeros(n_vibr_levels_total + num_atoms);
    // diffusion_RHS.zeros(num_molecules + num_atoms + 1);
    // diffusion_coeffs.zeros((n_vibr_levels_total + num_atoms) * (n_vibr_levels_total + num_atoms));
    // diffusion_rigid_rot_coeffs();
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  const arma::mat &kappa::Mixture::compute_diffusion_LHS(double T) {
    ...
    // for (i=0; i<num_molecules; i++) { 
    for (i=0; i<n_vibr_levels_total; i++) { // should be like this, george said!
     for (k=0; k<molecules[i].num_vibr_levels[0]; k++) { 
    ...
    // std::cout << diffusion_LHS;
    // std::cout << "DET = " << arma::det(diffusion_LHS)  << std::endl;
...
//  if (b==0) {
//   if (n==0) {
//    std::cout << "D RHS " <<  diffusion_RHS << std::endl;
//   }
//  }
 ...
 void kappa::Mixture::binary_diffusion(double T, kappa::models_omega model) {
  ...
  if (num_atoms==0) { // diatomic molecule mixture, pp. 158 Nagnibeda & Kustova, 2009.
   ...
   for (k = 0; k < molecules[0].num_vibr_levels[0]; k++) {
    ...
    binary_diff[k] = D_AA2 * (m_A/(rho/n)) * (m_A/(rho/n)) *
   	             ( (Y_A/D_A2) + (2./D_AA2) * ( (1./((molecules[0].mass * this_n_vl_mol[0][k])/rho) - Y_A - 1. ))) /
