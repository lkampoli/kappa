...
kappa::Mixture::Mixture(const std::vector<kappa::Molecule> &i_molecules, 
    ...
    : kappa::Approximation() {
        ...
        // FIXME: read the mixture and determine if there are e-
        is_ionized = false; // LC: to avoid e- loading
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  kappa::Mixture::Mixture(	const kappa::Molecule &molecule, 
    ...
    : kappa::Approximation() {
        ...
        // molecules = std::vector<kappa::Molecule>;
        molecules.push_back(molecule);
        num_molecules = 1;
        num_atoms = 0;
        cache_on = false;
  ...
  kappa::Mixture::Mixture(	const std::string particle_names, 
    ...
    : kappa::Approximation() {
        ...
        for (auto name : split_names) {
         ...
         if (name != "e-") { // if there is an electron, there should be charged particles, 
            	            // and the electron data will be loaded anyway
          try {
...
// end of Mixture constructors ////////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  void kappa::Mixture::init_matrices(const std::string &particles_filename) {
    ...
    diffusion_LHS.zeros(num_molecules + num_atoms + 1, num_molecules + num_atoms + 1); // new idea!
    //diffusion_RHS.zeros(n_vibr_levels_total + num_atoms);
    diffusion_RHS.zeros(num_molecules + num_atoms + 1);
    //diffusion_coeffs.zeros(n_vibr_levels_total + num_atoms);
    diffusion_coeffs.zeros(num_molecules + num_atoms + 1);
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  // this function is called by all the others in res
  // FIXME: valid only for rigid rotators, George?
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  const arma::mat &kappa::Mixture::compute_bulk_viscosity_LHS(double T, kappa::models_omega model) {
    ...
    /* Representation of the bulk viscosity matrix
      								    big block matrix
     ...............................................................................
     .                                                 sub-block matrix            .
     . | (0,0) (0,1) | ----------------------------- | (0,L_k) (0,L_k+1) |         .
     . | (1,0) (1,1) | 				     | (1,L_k) (1,L_k+1) |         .
     .        *     					      *	                   .
     .        *     					      *                    .
     .        *     					      *                    .
     . | (L_c,  0) (L_c,  1) | 			     | (L_c,  L_k) (L_c,  L_k+1) | .
     . | (L_c+1,0) (L_c+1,1) | --------------------- | (L_c+1,L_k) (L_c+1,L_k+1) | .
     .										   .
     ...............................................................................

       --- increasing book index k / increasing code index l
       *** increasing book index i / increasing code index k
      
        where L_k is the max. num. of vibr. levels of the 2nd molecule
        where L_c is the max. num. of vibr. levels of the 1st molecule

       ... we have a block for each couple of particles

    */

    // molecule + molecule collisions for molecules of different species or at different vibrational levels
    for (i = 0; i < num_molecules; i++) { 				// c
     for (j = i; j < num_molecules; j++) { 				// d
      ...
      for (k = 0; k < molecules[i].num_vibr_levels[0]; k++) { 	// i

       p1 = 2 * (vl_offset[i] + k);					// rows (in block of 2)
       o1 = vl_offset[i] + k;					
           
       for (l=0; l < molecules[j].num_vibr_levels[0]; l++) { 	// k
        //                   c  i                  d  k
        n_ij = this_n_vl_mol[i][k] * this_n_vl_mol[j][l];

        p2 = 2 * (vl_offset[j] + l);				// columns (in block of 2)
        o2 = vl_offset[j] + l;
    ...
    for (i = 0; i < num_molecules; i++) { 
       ...
       for (j = 0; j < num_molecules; j++) {
          ...
          A_cd   = 0.5   *      omega_22.at(i, j) / omega_11.at(i, j);	// eq. 5.62
          eta_cd = 0.625 * kT / omega_22.at(i, j); 			// 0.625 = 5/8
          rm     = 32    * n  * omega_22.at(i, j) / (5 * K_CONST_PI); 	// ???
          ...
          for (k = 0; k < molecules[i].num_vibr_levels[0]; k++) {
             ...
             for (l = 0; l < molecules[j].num_vibr_levels[0]; l++) {
              if ((k != l) || (i != j)) {               
                ...
                // eq. 5.78
                bulk_viscosity_LHS.at(p1    , p1    ) += 5 * kT * n_ij * coll_mass / ((molecules[i].mass + molecules[j].mass) * A_cd * eta_cd) 
       ...
       for (j = 0; j < num_atoms; j++) {
          ...
          A_cd   = 0.50  *      omega_22.at(i, num_molecules + j) / omega_11.at(i, num_molecules + j);	// eq. 5.62
          eta_cd = 0.625 * kT / omega_22.at(i, num_molecules + j); 					// 0.625 = 5/8
   ...
   for (i = 0; i < num_atoms; i++) { 
      ...
      for (j = 0; j < num_molecules; j++) {
         ...
         A_cd   = 0.50  *      omega_22.at(num_molecules + i, j) / omega_11.at(num_molecules + i, j);	// eq. 5.62
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  const arma::mat &kappa::Mixture::compute_bulk_viscosity_rigid_rot_LHS(double T, kappa::models_omega model) {
    ...
    // TODO: REMOVE
    // for (i=0; i < bulk_viscosity_rigid_rot_LHS.n_rows; i++) {
    //     for (j=0; j < bulk_viscosity_rigid_rot_LHS.n_cols; j++) {
    //         std::cout << bulk_viscosity_rigid_rot_LHS.at(i, j) << " ";
    //     }
    //     std::cout << std::endl;
    // }
    // std::cout << "BEFORE: " << arma::det(bulk_viscosity_rigid_rot_LHS * 1e15) << std::endl;
    ...
    // std::cout << "AFTER: " << arma::det(bulk_viscosity_rigid_rot_LHS * 1e15) << std::endl;
  ...
  const arma::mat &kappa::Mixture::compute_thermal_conductivity_LHS(double T, kappa::models_omega model) {
    ...
    for (i = 0; i < num_molecules; i++) {				// c
     for (j = i; j < num_molecules; j++) {				// d
        ...
        //std::cout << " D_cd -- mol+mol: " << D_cd << std::endl;
        rm   = ( 32  ) *  n * omega_22.at(i, j) / (5 * K_CONST_PI);	// ???
      ...
      for (k = 0; k < molecules[i].num_vibr_levels[0]; k++) {
       ...
       for (l = 0; l < molecules[j].num_vibr_levels[0]; l++) {
        ...
        for (j = 0; j < num_atoms; j++) {
           ...
           //std::cout << " D_cd -- mol+atom: " << D_cd << std::endl;
           D_cd_rot = D_cd;
           rm = 32 * n * omega_22.at(i, num_molecules + j) / (5 * K_CONST_PI);
     ...
     for (j = 0; j < num_atoms; j++) {
        ...
        for (k = 0; k < molecules[i].num_vibr_levels[0]; k++) {
           ...
           thermal_conductivity_LHS.at(p1 + 2, n_vibr_levels_total*3 + 2*j + 1) = -6 * T / (n * D_cd * K_CONST_PI) * A_cd * n_ij * molecules[i].mass * molecules[i].mass
                                                                                  * c_rot_arr(o1) / (rm * rot_rel_times.at(i, num_molecules + j) * (molecules[i].mass + molecules[j].mass)); // 0110 TODO fix brackets
    ...
    for (i = 0; i < num_atoms-1; i++) { 
     for (j = i+1; j < num_atoms; j++) {
        ...
        //std::cout << " D_cd -- atom+atom: " << D_cd << std::endl;
    ...
    for (i = 0; i < num_atoms; i++) { 
       ...
       for (j = 0; j < num_molecules; j++) {
          ...
          for (k = 0; k < molecules[j].num_vibr_levels[0]; k++) {
             ...
             thermal_conductivity_LHS.at(n_vibr_levels_total * 3 + 2 * i + 1, n_vibr_levels_total * 3 + 2 * i) -= 
      		0.75 * kT * n_ij * molecules[j].mass * (6 * C_cd - 5) / ((atoms[i].mass + molecules[j].mass) * n * D_cd); 					// 1000

             thermal_conductivity_LHS.at(n_vibr_levels_total * 3 + 2 * i    , n_vibr_levels_total * 3 + 2 * i + 1) -= 
 		0.75 * kT * n_ij * molecules[j].mass * (6 * C_cd - 5) / ((atoms[i].mass + molecules[j].mass) * n * D_cd); 					// 0100
...
//    check test ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
//    std::cout << "Entering in thermal_conductivity_LHS" << std::endl;
//    std::cout << "rows of LHS: " << thermal_conductivity_LHS.n_rows << std::endl;
//    std::cout << "cols of LHS: " << thermal_conductivity_LHS.n_cols << std::endl;
//    std::cout << "elem of LHS: " << thermal_conductivity_LHS.n_elem << std::endl;
//    int i1 = 0, i2 = 0;
//    std::cout << thermal_conductivity_LHS(i1  ,i2) << " " << thermal_conductivity_LHS(i1  ,i2+1) << " " << thermal_conductivity_LHS(i1  ,i2+2) << std::endl;
//    std::cout << thermal_conductivity_LHS(i1+1,i2) << " " << thermal_conductivity_LHS(i1+1,i2+1) << " " << thermal_conductivity_LHS(i1+1,i2+2) << std::endl;
//    std::cout << thermal_conductivity_LHS(i1+2,i2) << " " << thermal_conductivity_LHS(i1+2,i2+1) << " " << thermal_conductivity_LHS(i1+2,i2+2) << std::endl;
//
//    std::cout << thermal_conductivity_LHS(i1  ,i2+3) << " " << thermal_conductivity_LHS(i1  ,i2+4) << " " << thermal_conductivity_LHS(i1  ,i2+5) << std::endl;
//    std::cout << thermal_conductivity_LHS(i1+1,i2+3) << " " << thermal_conductivity_LHS(i1+1,i2+4) << " " << thermal_conductivity_LHS(i1+1,i2+5) << std::endl;
//    std::cout << thermal_conductivity_LHS(i1+2,i2+3) << " " << thermal_conductivity_LHS(i1+2,i2+4) << " " << thermal_conductivity_LHS(i1+2,i2+5) << std::endl;
//
//    std::cout << thermal_conductivity_LHS(i1  ,i2+6) << " " << thermal_conductivity_LHS(i1  ,i2+7) << " " << thermal_conductivity_LHS(i1  ,i2+8) << std::endl;
//    std::cout << thermal_conductivity_LHS(i1+1,i2+6) << " " << thermal_conductivity_LHS(i1+1,i2+7) << " " << thermal_conductivity_LHS(i1+1,i2+8) << std::endl;
//    std::cout << thermal_conductivity_LHS(i1+2,i2+6) << " " << thermal_conductivity_LHS(i1+2,i2+7) << " " << thermal_conductivity_LHS(i1+2,i2+8) << std::endl;
//
//    std::cout << thermal_conductivity_LHS(i1+3,i2) << " " << thermal_conductivity_LHS(i1+3,i2+1) << " " << thermal_conductivity_LHS(i1+3,i2+2) << std::endl;
//    std::cout << thermal_conductivity_LHS(i1+4,i2) << " " << thermal_conductivity_LHS(i1+4,i2+1) << " " << thermal_conductivity_LHS(i1+4,i2+2) << std::endl;
//    std::cout << thermal_conductivity_LHS(i1+5,i2) << " " << thermal_conductivity_LHS(i1+5,i2+1) << " " << thermal_conductivity_LHS(i1+5,i2+2) << std::endl;
//
//    std::cout << thermal_conductivity_LHS(i1+3,i2+3) << " " << thermal_conductivity_LHS(i1+3,i2+4) << " " << thermal_conductivity_LHS(i1+3,i2+5) << std::endl;
//    std::cout << thermal_conductivity_LHS(i1+4,i2+3) << " " << thermal_conductivity_LHS(i1+4,i2+4) << " " << thermal_conductivity_LHS(i1+4,i2+5) << std::endl;
//    std::cout << thermal_conductivity_LHS(i1+5,i2+3) << " " << thermal_conductivity_LHS(i1+5,i2+4) << " " << thermal_conductivity_LHS(i1+5,i2+5) << std::endl;
//    // ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

 
//    for ( j=0; j<thermal_conductivity_LHS.n_rows; j++ ) {
//     for ( i=0; i<thermal_conductivity_LHS.n_cols; i++ ) {
//    
//       std::cout << j << " " << i << " " << thermal_conductivity_LHS(j,i) << std::endl;
//     }
//    }
//    std::cout << "Exiting from thermal_conductivity_LHS" << std::endl;
...
// check test
//    std::cout << thermal_conductivity_coeffs.n_elem << std::endl;
//    std::cout << thermal_conductivity_coeffs.n_rows << std::endl;
//    std::cout << thermal_conductivity_coeffs.n_cols << std::endl;
//    for (int i=0; i<thermal_conductivity_coeffs.n_rows; i +=3) {
//     std::cout << i << " " << thermal_conductivity_coeffs(i) << std::endl;
//    }
// end check test
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  const arma::mat &kappa::Mixture::compute_thermal_conductivity_rigid_rot_LHS(double T, kappa::models_omega model) {
    ...
    for (i = 0; i < num_molecules - 1; i++) { // molecule + molecule collisions for different molecular species
        for (j = i+1; j < num_molecules; j++) {
            coll_mass = interactions[inter_index(i, j)].collision_mass;
            n_ij = this_n_molecules[i] * this_n_molecules[j];
            A_cd = 0.5 * omega_22.at(i, j) / omega_11.at(i, j);
    ...
    for (i = 0; i < num_molecules; i++) { // molecule + molecule collisions for identical molecular species
        ...
        thermal_conductivity_rigid_rot_LHS.at(3 * i + 1, 3 * i + 1) = 1.5 * kT * n_ij * A_cd * (2 + (20./3) * molecules[i].mass * c_rot_rigid_rot_arr[i]
    ...
    // std::cout << "BEFORE: " << arma::det(thermal_conductivity_rigid_rot_LHS * 1e40) << std::endl;
   ...
   // std::cout << "AFTER: " << arma::det(thermal_conductivity_rigid_rot_LHS * 1e40) << std::endl;
  ...
  const arma::mat &kappa::Mixture::compute_shear_viscosity_LHS(double T, kappa::models_omega model) {
    ...
    for (i1 = 0; i1 < num_molecules - 1; i1++) {
     for (i2 = i1+1; i2 < num_molecules; i2++) {
        ...
        // eq. 5.75 and eq. 5.62
        tmp = 3.2 * (this_n_molecules[i1] / n) * (this_n_molecules[i2] / n) * (molecules[i1].mass * molecules[i2].mass) * omega_22.at(i1, i2) / 
              (kT * (molecules[i1].mass + molecules[i2].mass) * (molecules[i1].mass + molecules[i2].mass)); // 3.2 = 16/5
  ...
  const arma::mat &kappa::Mixture::compute_diffusion_LHS(double T) {
    ...
    for (i=0; i<num_molecules; i++) { 
     for (k=0; k<molecules[i].num_vibr_levels[0]; k++) { 
      ...
      //std::cout << " diffusion_LHS -- molecules = " << thermal_conductivity_LHS.at(i * 3, k * 3) << std::endl;
     }
     ...
     for (k=0; k<num_atoms; k++) {	
      ...
      //std::cout << " diffusion_LHS -- molecules = " << thermal_conductivity_LHS.at(i * 3, n_vibr_levels_total * 3 + k * 2) << std::endl;
     }
    }
    ...
    for (i=0; i<num_molecules; i++) {
     for (k=0; k<molecules[i].num_vibr_levels[0]; k++) {
      diffusion_LHS.at(0,offset) = this_n_vl_mol[i][k] * molecules[i].mass / (this_total_n * 1e24);
      //diffusion_LHS.at(0,offset) = this_n_vl_mol[i][k] * molecules[i].mass / (this_total_n);
      offset += 1;
     }
    }
   
    for (i=0; i<num_atoms; i++) {
     diffusion_LHS.at(0,offset) = this_n_atom[i] * atoms[i].mass / (this_total_n * 1e24);
     //diffusion_LHS.at(0,offset) = this_n_atom[i] * atoms[i].mass / (this_total_n);
     offset += 1;
    }
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  const arma::mat &kappa::Mixture::lite_compute_diffusion_LHS(double T, int b, int n, std::string ParticleType) {
    ...
    double n_tot = this_total_n;
    double A_cd, B_cd, C_cd, D_cd, coll_mass, D_cd_rot, n_ij, rm, kT=K_CONST_K * T; 
    int c, d, i, j, k, l, p1, p2, o1, o2, row=0, col=0, offset=0;

  if (ParticleType == "MOL") {

// first row, first column -- (0,0)
    for (c=0; c<num_molecules; c++) {
     if (c==b) {
      for (i=0; i<molecules[c].num_vibr_levels[0]; i++) {
       if (i==n) {
        n_ij = this_n_vl_mol[c][i] * this_n_vl_mol[b][n];
        coll_mass = interactions[inter_index(c, b)].collision_mass;
        D_cd = (3./16) * kT/(n_tot * coll_mass * omega_11.at(c, b)); 
        diffusion_LHS.at(row, col) = -1.5 * kT * n_ij / (n_tot * D_cd); // N2_1 x N2_1
        std::cout << " diffusion_LHS.at("<<row<<","<<col<<")" << " " << diffusion_LHS.at(row, col) << std::endl;
       }
      }
     }
    }
...
// second row, first column -- (1,0)
     ...
     if (c==b) {
      for (i=0; i<molecules[c].num_vibr_levels[0]; i++) {
       if (i!=n) {
        ...
        coll_mass = interactions[inter_index(c, b)].collision_mass;
        D_cd = (3./16) * kT/(n_tot * coll_mass * omega_11.at(c, b)); 
        diffusion_LHS.at(row, col) += -1.5 * kT * n_ij / (n_tot * D_cd); // N2_1 x N2_2 + ... + N2_1 x N2_max
        std::cout << " diffusion_LHS.at("<<row<<","<<col<<")" << " " << diffusion_LHS.at(row, col) << std::endl;
       }
      }
     }
    ...
    //std::cout << " diffusion_LHS.size " << diffusion_LHS.n_elem << std::endl;
    //std::cout << " diffusion_LHS.size " << diffusion_LHS.n_rows << std::endl;
    //std::cout << " diffusion_LHS.size " << diffusion_LHS.n_cols << std::endl;
    //std::cout << " row " << row << std::endl;

// check if all molecules are considered
    while (row<num_molecules) {
...
// mol + mol
     for (c=0; c<num_molecules; c++) {
      if (c!=b) {
       ...
       for (i=0; i<molecules[c].num_vibr_levels[0]; i++) {
        ...
        coll_mass = interactions[inter_index(c, b)].collision_mass;
        D_cd = (3./16) * kT/(n_tot * coll_mass * omega_11.at(c, b)); 
        diffusion_LHS.at(row, col) += -1.5 * kT * n_ij / (n_tot * D_cd); // O2_1 x N2_1 ... O2_max x N2_1
									 // NO_1 x N2_1 ... NO_max x N2_1 if present NO
        std::cout << " diffusion_LHS.at("<<row<<","<<col<<")" << " " << diffusion_LHS.at(row, col) << std::endl;
       }
      }
     }
    ...
    // we reached num_molecules, so consider atoms
    for (c=0; c<num_atoms; c++) {
     //if (c!=b) { // already comprised by the initial if
      row++;
      n_ij = this_n_atom[c] * this_n_vl_mol[b][n];
      coll_mass = interactions[inter_index(c, b)].collision_mass;
      D_cd = (3./16) * kT/(n_tot * coll_mass * omega_11.at(c, b)); 
      diffusion_LHS.at(row, col) = -1.5 * kT * n_ij / (n_tot * D_cd); // N x N2_i
      std::cout << " diffusion_LHS.at("<<row<<","<<col<<")" << " " << diffusion_LHS.at(row, col) << std::endl;
     //}
    }

// reset index, point to second column of the system 5.104-5.107
    row=0, col++;
...
// first row, second column -- (0,1)
    for (c=0; c<num_molecules; c++) {
     if (c==b) {
      for (i=0; i<molecules[c].num_vibr_levels[0]; i++) {
       if (i==n) {
        for (k=0; k<molecules[c].num_vibr_levels[0]; k++) {
         if (k!=n) {
          n_ij = this_n_vl_mol[c][i] * this_n_vl_mol[b][k];
          coll_mass = interactions[inter_index(c, b)].collision_mass;
          D_cd = (3./16) * kT/(n_tot * coll_mass * omega_11.at(c, b)); 
          diffusion_LHS.at(row, col) += -1.5 * kT * n_ij / (n_tot * D_cd); // N2_1 x N2_2 + ... + N2_1 x N2_max 
          std::cout << " diffusion_LHS.at("<<row<<","<<col<<")" << " " << diffusion_LHS.at(row, col) << std::endl;
       ...
       if (i!=n) {
        for (k=0; k<molecules[c].num_vibr_levels[0]; k++) {
         if (k!=n) {
          ...
          coll_mass = interactions[inter_index(c, b)].collision_mass;
          D_cd = (3./16) * kT/(n_tot * coll_mass * omega_11.at(c, b)); 
          diffusion_LHS.at(row, col) += -1.5 * kT * n_ij / (n_tot * D_cd); // N2_2 x N2_2 + ... + N2_2 x N2_max +
									   // N2_max x N2_2 + ... + N2_max x N2_max 
          std::cout << " diffusion_LHS.at("<<row<<","<<col<<")" << " " << diffusion_LHS.at(row, col) << std::endl;
         }
        }
       }
...
// (b = mol) + mol
     for (c=0; c<num_molecules; c++) {
      if (c!=b) {
       row++;
       for (i=0; i<molecules[c].num_vibr_levels[0]; i++) {
        for (k=0; k<molecules[b].num_vibr_levels[0]; k++) {
         if (k!=n) {
          n_ij = this_n_vl_mol[c][i] * this_n_vl_mol[b][k];
          coll_mass = interactions[inter_index(c, b)].collision_mass;
          D_cd = (3./16) * kT/(n_tot * coll_mass * omega_11.at(c, b)); 
          diffusion_LHS.at(row, col) += -1.5 * kT * n_ij / (n_tot * D_cd); // O2_1 x N2_2 + ... + O2_max x N2_2 ...
									   // O2_1 x N2_max + ... + O2_max x N2_max  
          std::cout << " /\ diffusion_LHS.at("<<row<<","<<col<<")" << " " << diffusion_LHS.at(row, col) << std::endl;
         }
        }
       }
...
// (b = mol) + at 
    for (c=0; c<num_atoms; c++) {
     //if (c!=b) { // already comprised by the initial if
      row++;
      for (i=0; i<molecules[b].num_vibr_levels[0]; i++) {
       n_ij = this_n_atom[c] * this_n_vl_mol[b][i];
       coll_mass = interactions[inter_index(c, b)].collision_mass;
       D_cd = (3./16) * kT/(n_tot * coll_mass * omega_11.at(c, b)); 
       diffusion_LHS.at(row, col) += -1.5 * kT * n_ij / (n_tot * D_cd); 
      }
      std::cout << " __diffusion_LHS.at("<<row<<","<<col<<")" << " " << diffusion_LHS.at(row, col) << std::endl;
     //}
    } 

// reset index, pointing at the third column of the system
    row=0, col++;
    //std::cout << " col " << col << std::endl;

// first row, third column -- (0,2)
    for (c=0; c<num_molecules; c++) {
     if (c==b) {
      for (i=0; i<molecules[c].num_vibr_levels[0]; i++) {
       if (i==n) {
        for (d=0; d<num_molecules; d++) {
         if (d!=b) {
          for (k=0; k<molecules[d].num_vibr_levels[0]; k++) {
           n_ij = this_n_vl_mol[c][i] * this_n_vl_mol[d][k];
           coll_mass = interactions[inter_index(c, d)].collision_mass;
           D_cd = (3./16) * kT/(n_tot * coll_mass * omega_11.at(c, d)); 
           diffusion_LHS.at(row, col) += -1.5 * kT * n_ij / (n_tot * D_cd); // N2_1 x O2_1 + ... + N2_1 x O2_max ... +
									    // N2_1 x NO_1 + ... + N2_1 x NO_max
           diffusion_LHS.at(col, row) = diffusion_LHS.at(row, col); // symmetrization
           std::cout << " ** diffusion_LHS.at("<<row<<","<<col<<")" << " " << diffusion_LHS.at(row, col) << std::endl;
           std::cout << " ** diffusion_LHS.at("<<col<<","<<row<<")" << " " << diffusion_LHS.at(col, row) << std::endl;
          }
         }
        }
       }
        ...
        for (d=0; d<num_molecules; d++) {
         if (d!=b) {
          for (k=0; k<molecules[d].num_vibr_levels[0]; k++) {
           ...
           coll_mass = interactions[inter_index(c, d)].collision_mass;
           D_cd = (3./16) * kT/(n_tot * coll_mass * omega_11.at(c, d)); 
           diffusion_LHS.at(row, col) += -1.5 * kT * n_ij / (n_tot * D_cd); // N2_2 x O2_1 + ... + N2_2 x O2_max ... +
									    // N2_max x O2_1 + ... + N2_max x O2_max
           std::cout << " ~ diffusion_LHS.at("<<row<<","<<col<<")" << " " << diffusion_LHS.at(row, col) << std::endl;
          }
         }
        }
...
// mol + atom
    for (c=0; c<num_atoms; c++) {
     //if (c!=b) { // already comprised in the initial if
      row++;
      for (d=0; d<num_molecules; d++) {
       if (d!=b) {
        for (k=0; k<molecules[d].num_vibr_levels[0]; k++) {
         ...
         coll_mass = interactions[inter_index(c, d)].collision_mass;
         D_cd = (3./16) * kT/(n_tot * coll_mass * omega_11.at(c, d)); 
         diffusion_LHS.at(row, col) = -1.5 * kT * n_ij / (n_tot * D_cd); // N x O2_1 + ... + N x O2_max
									 // O x O2_1 + ... + O x O2_max
         std::cout << " ++ diffusion_LHS.at("<<row<<","<<col<<")" << " " << diffusion_LHS.at(row, col) << std::endl;
        }
       }
...
// atom + atom
    for (c=0; c<num_atoms; c++) {
     //if (c!=b) {
      //row=0, col=0;
      row++;
      for (d=0; d<num_atoms; d++) {
       n_ij = this_n_atom[c] * this_n_atom[d];
       coll_mass = interactions[inter_index(c, d)].collision_mass;
       D_cd = (3./16) * kT/(n_tot * coll_mass * omega_11.at(c, d));
       diffusion_LHS.at(row, col) = -1.5 * kT * n_ij / (n_tot * D_cd); // N x N
								       // N x O	
								       // O x N
								       // O x O
       std::cout << " -- diffusion_LHS.at("<<row<<","<<col<<")" << " " << diffusion_LHS.at(row, col) << std::endl;
...
//          coll_mass = interactions[inter_index(c, d)].collision_mass;
//          D_cd = (3./16) * kT/(n_tot * coll_mass * omega_11.at(c, d)); 
//          diffusion_LHS.at(row, col+2) += -1.5 * kT * n_ij / (n_tot * D_cd); //(0,2)
//         }
//        }
//       } 
//       
//       for (d=0; d<num_atoms; d++) {
//        if (d!=b) {
//         n_ij = this_n_vl_mol[c][i] * this_n_atom[d];
//         coll_mass = interactions[inter_index(c, d)].collision_mass;
//         D_cd = (3./16) * kT/(n_tot * coll_mass * omega_11.at(c, d)); 
//         diffusion_LHS.at(row, col) += -1.5 * kT * n_ij / (n_tot * D_cd); 
//         col++;
...
//      }
//      
//       for (l=0; l<molecules[b].num_vibr_levels[0]; l++) { // vibr. levels, k
//        if (i==b && k!=n && l!=n) {
//         n_ij = this_n_vl_mol[i][k] * this_n_vl_mol[b][l]; // ostoroshno! to the index
//         coll_mass = interactions[inter_index(i, b)].collision_mass;
//         D_cd = (3./16) * kT/(n_tot * coll_mass * omega_11.at(i, b));
//         diffusion_LHS.at(i, k+1) += -1.5 * kT * n_ij / (n_tot * D_cd); 
//        }
//       }
//
//      for (c=0; c<num_molecules; c++) {
//       if (i!=b) {
//       if (i!=b && k!=n) {
//        for (l=0; l<molecules[b].num_vibr_levels[0]; l++) {
//         if (k!=n) { //FIXME
//          n_ij = this_n_vl_mol[i][k] * this_n_vl_mol[b][l]; // ostoroshno to the index
//          coll_mass = interactions[inter_index(i, b)].collision_mass;
//          D_cd = (3./16) * kT/(n_tot * coll_mass * omega_11.at(i, b));
//          diffusion_LHS.at(i+1, k+2) += -1.5 * kT * n_ij / (n_tot * D_cd);
//         }
//        }
//       }
//      }
//     }
//
//      // third line ... FIXME change all indexe in diffusion_LHS to correctly fill the matrix
//      if (i!=b) {
//       diffusion_LHS.at(i+2, k) += -1.5 * kT * n_ij / (n_tot * D_cd);
//      }
//      
//      for (l=0; l<molecules[b].num_vibr_levels[0]; l++) {
//       if (i!=b && l!=n) {
//        diffusion_LHS.at(i+2, k+1) += -1.5 * kT * n_ij / (n_tot * D_cd);
//       } 
//      }
//    
//      
...
//     }
    ...
    // Auxiliary normalization condition, eq. 5.107
    // ============================================
    std::cout << " Impose auxiliary normalization condition " << std::endl;
    diffusion_LHS.at(0, 0) = this_n_vl_mol[b][n] * molecules[b].mass / this_total_dens;
    diffusion_LHS.at(0, 1) = (this_n_molecules[b] - this_n_vl_mol[b][n]) * molecules[b].mass / this_total_dens;
    std::cout << diffusion_LHS.at(0, 0) << std::endl;
    std::cout << diffusion_LHS.at(0, 1) << std::endl;

    if (ParticleType == "MOL") {
     for (i=0; i<num_molecules; i++) { // molecules, d
      if (i!=b) {
       diffusion_LHS.at(0, 2+i) = this_n_molecules[i] * molecules[i].mass;
       std::cout << " diffusion_LHS.at(0, 2+i) " << diffusion_LHS.at(0, 2+i) << std::endl;
      }
     }
  
     for (i=0; i<num_atoms; i++) { // atoms, d
      //if (i!=b) {
      diffusion_LHS.at(0, 1+num_molecules+i) = this_n_atom[i] * atoms[i].mass;
      std::cout << " diffusion_LHS.at(0, 1+num_molecules+i) " << diffusion_LHS.at(0, 1+num_molecules+i) << std::endl;
      //}
     }
    }
  
    // TODO atoms are not missing ?

    //diffusion_LHS.at(0, 3) /= this_total_dens;

    std::cout << " Divide by total density & print " << std::endl;
    diffusion_LHS /= this_total_dens;
    std::cout << " diffusion_LHS " << diffusion_LHS << std::endl;
    ...
    */ 
    ...
    for (i=0; i<num_molecules; i++) { // molecules      	
     for (k=0; k<molecules[i].num_vibr_levels[0]; k++) { // vibr. levels
      ...
      //std::cout << " diffusion_RHS -- molecules = " << diffusion_RHS.at(offset) << std::endl;
      offset += 1;
     }
    }
    ...
    for (i=0; i<num_atoms; i++) { // over atoms
     ...
     //std::cout << " diffusion_RHS -- atoms = " << diffusion_RHS.at(offset) << std::endl;
     offset += 1;
    }
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  /*
    Computes RHS for diffusion
    from eq. 5.104-106 for fixed (c,i,b,n) we have
    rhs(c,i,b,n) = 3kT * (1 - rho_bn / rho) for c=b, i=n
    rhs(c,i,b,n) = -3kT * ( (rho_b - rho_bn) / rho) for c=b, i=0,...,n-1, n+1,...,L_b
    rhs(c,i,b,n) = -3kT * (rho_c / rho) for c=1,...,b-1, b+1,...,L, i=0,...,L_c
    for b=1,...,L and n=0,...,L_b
    
    So this computes the RHS for a fixed b (# of particle) and a fixed n (# of vibrational level)
  */ 

  int c, i, k, offset=0;
  ///std::cout << " (b,n) " << " " << ParticleType << " " << b << " " << n << std::endl;

// work-around to fil up the RHS according to the ParticleType
  if (ParticleType == "MOL") {

// this first row of the RHS vector will be replaced by the linear independence condition (see below)
    diffusion_RHS.at(offset) = 1. - (this_n_vl_mol[b][n] * molecules[b].mass / this_total_dens);
    std::cout << " diffusion_RHS.at("<<offset<<")" << " " << diffusion_RHS.at(offset) << std::endl;
    offset++;

// eq. 5.105
// the outer loop on species can be commented because according to eq. 5.105 we consider only c=b
// for (c=0; c<num_molecules; c++) { 
    for (i=0; i<molecules[b].num_vibr_levels[0]; i++) {       
     if (i!=n) {
      diffusion_RHS.at(offset) = - (this_n_molecules[b] - this_n_vl_mol[b][i]) * molecules[b].mass / this_total_dens;
      std::cout << " diffusion_RHS.at("<<offset<<")" << " " << diffusion_RHS.at(offset) << std::endl;
      offset++;
     }
    }

// eq. 5.106 for molecules
    for (c=0; c<num_molecules; c++) { 
     if (c!=b) {
// the inner loop on vibrational level can be commented since according to eq. 5.106 we consider only species
      //for (i=0; i<molecules[c].num_vibr_levels[0]; i++) {       
       //if (i!=n) {
        //diffusion_RHS.at(1+molecules[b].num_vibr_levels[0]+c) = - this_n_molecules[c] * molecules[c].mass / this_total_dens;
        diffusion_RHS.at(offset) = - this_n_molecules[c] * molecules[c].mass / this_total_dens;
        std::cout << " diffusion_RHS.at("<<offset<<")" << " " << diffusion_RHS.at(offset) << std::endl;
        offset++;
       //}
      //}
...
// atoms
    for (c=0; c<num_atoms; c++) { 
     diffusion_RHS.at(offset) = - this_n_atom[c] * atoms[c].mass / this_total_dens;
     std::cout << " diffusion_RHS.at("<<offset<<")" << " " << diffusion_RHS.at(offset) << std::endl;
     offset++;
    }
  }
// entry particle type --> atom
  else if (ParticleType == "AT") {
    offset=0;
 
    diffusion_RHS.at(offset) = 1. - (this_n_atom[b] * atoms[b].mass / this_total_dens);
    std::cout << " diffusion_RHS.at("<<offset<<")" << " " << diffusion_RHS.at(offset) << std::endl;
    offset++;

// eq. 5.105
// the outer loop on species can be commented because according to eq. 5.105 we consider only c=b
// the inner loop on vibrational levels must not be present for atoms
// for (c=0; c<num_molecules; c++) { 
//  for (i=0; i<molecules[b].num_vibr_levels[0]; i++) {       
//   if (i!=n) {
      diffusion_RHS.at(offset) = 0.;
      std::cout << " diffusion_RHS.at("<<offset<<")" << " " << diffusion_RHS.at(offset) << std::endl;
      offset++;
//   }
//  }
...
// molecules, eq. 5.106
    for (c=0; c<num_molecules; c++) { 
// outer branching not necessary since particle type is passed in input
//   if (c!=b) {
// the inner loop on vibrational level can be commented since according to eq. 5.106 we consider only species
//    for (i=0; i<molecules[c].num_vibr_levels[0]; i++) {       
// inner branching not necessary because b=atom and so n=0
       //if (i!=n) {
        diffusion_RHS.at(offset) = - this_n_molecules[c] * molecules[c].mass / this_total_dens;
        std::cout << " diffusion_RHS.at("<<offset<<")" << " " << diffusion_RHS.at(offset) << std::endl;
        offset++;
...
// atoms
   ...
   std::cout << " ParticleType not allowed! " << std::endl;
  }

// adding the missing factor
  diffusion_RHS *= 3 * K_CONST_K * T; 

// auxiliary normalization condition
  diffusion_RHS[0] = 0.;

  std::cout << " diffusion_RHS " << diffusion_RHS << std::endl;
  return diffusion_RHS;
  }
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  const arma::vec &kappa::Mixture::compute_diffusion_coeffs(double T, int b, int n) {
    ...
    //std::cout << " compute_diffusion_coeffs::compute_diffusion_RHS(T, b, n) >> " << std::endl;
    compute_diffusion_RHS(T, b, n);
    //std::cout << " compute_diffusion_coeffs::compute_diffusion_RHS(T, b, n) << " << std::endl;

    //std::cout << " compute_diffusion_coeffs::arma::solve >> " << std::endl;
    ...
    //std::cout << " Diffusion coefficients: " << diffusion_coeffs << std::endl;
    //std::cout << " compute_diffusion_coeffs::arma::solve << " << std::endl;

// check ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
//    std::cout << " Entering in diffusion_coeffs " << std::endl;
//    std::cout << "  rows: " << diffusion_coeffs.n_rows << std::endl;
//    std::cout << "  cols: " << diffusion_coeffs.n_cols << std::endl;
//    std::cout << "  elem: " << diffusion_coeffs.n_elem << std::endl;
//    std::cout << " Diffusion coefficients: " << diffusion_coeffs << std::endl;
//    std::cout << " Exiting from diffusion_coeffs " << std::endl;
// check ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

    //return diffusion_coeffs/(2 * this_total_n);
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  const arma::vec &kappa::Mixture::lite_compute_diffusion_coeffs(double T, int b, int n, std::string ParticleType) {
    ...
    //lite_compute_diffusion_RHS(T, b, n, ParticleType);
    //std::cout << " diffusion_RHS " << diffusion_RHS << std::endl;
    //lite_compute_diffusion_LHS(T, b, n);
    //std::cout << " diffusion_LHS " << diffusion_LHS << std::endl;
    //diffusion_coeffs = arma::solve(diffusion_LHS * 1e40, diffusion_RHS * 1e20);
    //std::cout << " diffusion_coeffs " << diffusion_coeffs << std::endl;
    //return diffusion_coeffs;
  }
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  void kappa::Mixture::diffusion(double T) {
    ...
    //std::cout << " diffusion::compute_diffusion_LHS(T) >> " << std::endl;
    ...
    for (i=0; i<num_molecules; i++) { // molecules, b in eq. 5.19
     for (k=0; k<molecules[i].num_vibr_levels[0]; k++) { // vibr. levels, n
      ...
      //std::cout << " diff.col -- mol = " << diff.col(offset) << std::endl;
      //std::cout << " diff.col -- mol = " << T << " " << i << " " << k << " " << compute_diffusion_coeffs(T, i, k) << std::endl;
      offset += 1;
     }
    }
    for (i=0; i<num_atoms; i++) {
     diff.col(offset) = compute_diffusion_coeffs(T, num_molecules + i, 0);
     //std::cout << " diff.col -- at = " << diff.col(offset) << std::endl;
     offset += 1;
    }

    diffusion_coeffs /= (2 * this_total_n); // D_cidk of eq. 5.8
    //std::cout << " 2n = " << 2 * this_total_n << std::endl;
    //std::cout << " diff = " << diff/(2 * this_total_n) << std::endl;
    //std::cout << " diffusion coeffs = " << diffusion_coeffs.size() << std::endl;
    //std::cout << " diffusion coeffs = " << diffusion_coeffs << std::endl;
 
  
    
  
// check test, eq. 5.18 FIXME bugs
...
//      test0 += 	(molecules[0].mass * this_n_vl_mol[0][0]/rho) * diff(0,0) + 
//      		(molecules[0].mass * this_n_vl_mol[0][1]/rho) * diff(0,1) + 
//      		(atoms[0].mass * this_n_atom[0]/rho) * diff(0,2) + 
//      		(molecules[0].mass * this_n_vl_mol[0][0]/rho) * diff(1,0) + 
//      		(molecules[0].mass * this_n_vl_mol[0][1]/rho) * diff(1,1) + 
//      		(atoms[0].mass * this_n_atom[0]/rho) * diff(1,2) + 
//      		(molecules[0].mass * this_n_vl_mol[0][0]/rho) * diff(2,0) + 
//      		(molecules[0].mass * this_n_vl_mol[0][1]/rho) * diff(2,1) + 
//      		(atoms[0].mass * this_n_atom[0]/rho) * diff(2,2);

//      test0 += (molecules[i].mass * this_n_vl_mol[i][k]/rho) * diff(offset,k)
//      test0 += (molecules[i].mass * this_n_vl_mol[i][k]/rho) * diff(offset,0) + 
//	       (molecules[i].mass * this_n_vl_mol[i][k]/rho) * diff(offset,1) + 
//	       (molecules[i].mass * this_n_vl_mol[i][k]/rho) * diff(offset,2);
//      offset += 1;
//     }
//    }
...
// molecules
   std::cout << " lite_diffusion --> molecules " << std::endl;
   for (i=0; i<num_molecules; i++) {
    for (k=0; k<molecules[i].num_vibr_levels[0]; k++) {
     std::cout << " lite_compute_diffusion_RHS " << std::endl;
     lite_compute_diffusion_RHS(T, i, k, "MOL");
     std::cout << " lite_compute_diffusion_LHS " << std::endl;
     lite_compute_diffusion_LHS(T, i, k, "MOL");
     //diffusion_coeffs = arma::solve(diffusion_LHS, diffusion_RHS); 
     std::cout << " IN: arma::solve(diffusion_LHS, diffusion_RHS) " << std::endl;
     arma::solve(diffusion_LHS, diffusion_RHS) / (2. * this_total_n); 
     //lite_diff.col(offset) = arma::solve(diffusion_LHS, diffusion_RHS) / (2. * this_total_n); 
     //std::cout << " OUT: arma::solve(diffusion_LHS, diffusion_RHS)  " << std::endl;
     std::cout <<  arma::solve(diffusion_LHS, diffusion_RHS) / (2. * this_total_n)  << std::endl;
     //diffusion_coeffs /= (2 * this_total_n); 
     //std::cout << " diffusion coeffs = " << diffusion_coeffs << std::endl;
     //std::cout << " diffusion coeffs = " << lite_diff.col(offset) << std::endl;
     //offset++;
    }
   }
...
// atoms
//   offset=0;
//   std::cout << " lite_diffusion --> atoms " << std::endl;
//   for (i=0; i<num_atoms; i++) {
//    lite_compute_diffusion_RHS(T, num_molecules + i, 0, "AT");
//    lite_compute_diffusion_LHS(T, num_molecules + i, 0, "AT");
//    //diffusion_coeffs = arma::solve(diffusion_LHS, diffusion_RHS); 
//    lite_diff.col(offset) = arma::solve(diffusion_LHS, diffusion_RHS) / (2. * this_total_n);
//    //diffusion_coeffs /= (2 * this_total_n); 
//    //std::cout << " diffusion coeffs = " << diffusion_coeffs << std::endl;
...
//    for (i=0; i<num_molecules; i++) { 
//     for (k=0; k<molecules[i].num_vibr_levels[0]; k++) { 
//      lite_diff.col(offset) = lite_compute_diffusion_coeffs(T, i, k, "MOL");
//      offset += 1;
//     }
//    }
//
...
//    std::cout << " lite_diffusion --> atoms " << std::endl;
//    for (i=0; i<num_atoms; i++) {
//     lite_diff.col(offset) = lite_compute_diffusion_coeffs(T, num_molecules + i, 0, "AT");
//     offset += 1;
//    }
//
//    diffusion_coeffs /= (2 * this_total_n); 
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  // Semplified version for diffusion coefficients computation 
  arma::mat kappa::Mixture::binary_diffusion(double T, kappa::models_omega model) {
  std::cout << " Simplified version for diffusion coefficients computation -- start! " << std::endl; 

    // simplified model for binary mixtures
    // ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
     int i, j, c, k, d;
     double n      = this_total_n;
     //n_ij = this_n_molecules[i] * this_n_atom[j];
     ...
     //double D_AA2  = (3./16) * (K_CONST_K * T)/(n * coll_mass * omega_11.at(0, 0));
     ...
     //double D_A2   = (3./8.) * (K_CONST_K * T)/(n * coll_mass_A2_A2 * omega_11.at(0, 0));
     //double D_A2   = (3./8.) * (K_CONST_K * T)/(n * coll_mass * omega_11.at(0, 0));

//     std::cout << " num_molecules " << num_molecules << std::endl;
//     std::cout << " rho " << rho << std::endl;
//     std::cout << " rho_A2 " << rho_A2 << std::endl;
//     std::cout << " rho_A " << rho_A << std::endl;
//     std::cout << " Y_A2 " << Y_A2 << std::endl;
//     std::cout << " Y_A " << Y_A << std::endl;
//     std::cout << " D_AA2 " << D_AA2 << std::endl;
//     std::cout << " D_A2 " << D_A2 << std::endl;
//     std::cout << " omega_11.at(0, 0) " << omega_11.at(0, 0) << std::endl;

     diff.zeros(n_vibr_levels_total+num_atoms,n_vibr_levels_total+num_atoms); // we don't care about the other coeffs
     //std::cout << " diff " << diff << std::endl;

      std::cout << " Diatomic molecule mixture " << std::endl;
      for (c = 0; c < num_molecules; c++) {
       for (d = 0; d < num_molecules; d++) {
        for (i = 0; i < molecules[c].num_vibr_levels[0]; i++) {
         for (k = 0; k < molecules[d].num_vibr_levels[0]; k++) {
            
          if (k==i) { // at the same vibrational level
           diff(i,k) = D_A2 * ( 1./((molecules[d].mass * this_n_vl_mol[d][k])/rho) -1 );
//           std::cout << " diff(" << i << "," << k << ") " << diff(i,k) << std::endl;
          }
          else { // at different vibrational level
           diff(i,k) = -D_A2;
//           std::cout << " diff(" << i << "," << k << ") " << diff(i,k) << std::endl;
          }
//          std::cout << " diff(" << i << "," << k << ") " << diff(i,k) << std::endl;
     ...
     if ( num_atoms == 0 ) { // diatomic molecule mixture
     ...
     std::cout << " N2/N or O2/O mixture " << std::endl;


      // molecule + molecule interaction
      for (c = 0; c < num_molecules; c++) {
       for (d = 0; d < num_molecules; d++) {
        //double D_cd = (3./16.) * (K_CONST_K * T)/(n * coll_mass * omega_11.at(c,d));
        //std::cout << " D_cd " << D_cd << std::endl;
        for (i = 0; i < molecules[c].num_vibr_levels[0]; i++) {
         for (k = 0; k < molecules[d].num_vibr_levels[0]; k++) {

          if (k==i) { // at the same vibrational level
           diff(i,k) = D_AA2 * (m_A/(rho/n)) * (m_A/(rho/n)) *
                       ( (Y_A/D_A2) + (2./D_AA2) * ( (1./((molecules[d].mass * this_n_vl_mol[d][k])/rho) - Y_A - 1. ) ) ) /
                       ( (Y_A2/(2.*D_A2)) + (Y_A/D_AA2) ); 
//           std::cout << " diff(" << i << "," << k << ") " << diff(i,k) << std::endl;
          }
          else { // at different vibrational level
           diff(i,k) = D_AA2 * (m_A/(rho/n)) * (m_A/(rho/n)) * ( (Y_A/D_A2) - (2./D_AA2) * (Y_A + 1.) ) / ( (Y_A2/(2.*D_A2)) + (Y_A/D_AA2) );
           //diff(i,k) = D_AA2 * (m_A/(rho/this_n_vl_mol[d][k])) * (m_A/(rho/this_n_vl_mol[d][k])) * ( (Y_A/D_A2) - (2./D_AA2) * (Y_A + 1.) ) / ( (Y_A2/(2.*D_A2)) + (Y_A/D_AA2) );
//           std::cout << " diff(" << i << "," << k << ") " << diff(i,k) << std::endl;
          }
         }
        }
      ...
      for (c = 0; c < num_molecules; c++) {
       for (i = 0; i < molecules[c].num_vibr_levels[0]; i++) {
        diff(i, n_vibr_levels_total+num_atoms-1) = -D_AA2 * (m_A2 * m_A) / ((rho/n) * (rho/n));
        //diff(i, n_vibr_levels_total+num_atoms-1) = -D_AA2 * (m_A2 * m_A) / ((rho/this_n_vl_mol[c][i]) * (rho/this_n_vl_mol[c][i]));

        // simmetrization
        diff(n_vibr_levels_total+num_atoms-1,i) = diff(i, n_vibr_levels_total+num_atoms-1);
//        std::cout << " diff(" << i << "," << n_vibr_levels_total+num_atoms-1 << ") " << diff(i,n_vibr_levels_total+num_atoms-1) << std::endl;
       }
      } 
 
      // atom + atom
      diff(n_vibr_levels_total+num_atoms-1, n_vibr_levels_total+num_atoms-1) = D_AA2 * ( (m_A2 * m_A) / ((rho/n) * (rho/n))) * ((1./Y_A) -1.);
//      std::cout << " diff(" << n_vibr_levels_total+num_atoms-1 << "," << n_vibr_levels_total+num_atoms-1 << ") " 
//     	  	<< diff(n_vibr_levels_total+num_atoms-1, n_vibr_levels_total+num_atoms-1) << std::endl;
     }   
     std::cout << " diff-IDA " << diff << std::endl;
  }
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  void kappa::Mixture::compute_transport_coefficients(  double T, 
    ...
    // extraction of transport coeffiecients' value(s)
