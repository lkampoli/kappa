...
kappa::Mixture::Mixture(const std::vector<kappa::Molecule> &i_molecules, const std::vector<kappa::Atom> &i_atoms, const std::string &interactions_filename, const std::string &particles_filename):kappa::Approximation() {
    // cannot have ionized!
...
kappa::Mixture::Mixture(const std::vector<kappa::Molecule> &i_molecules, const std::string &interactions_filename, const std::string &particles_filename):kappa::Approximation() {
    // cannot have ionized! 
    molecules = i_molecules;
    num_molecules = molecules.size();
    num_atoms = 0;
...
kappa::Mixture::Mixture(const std::vector<kappa::Atom> &i_atoms, const std::string &interactions_filename, const std::string &particles_filename):kappa::Approximation() {
    // cannot have ionized!
    atoms = i_atoms;
    num_molecules = 0;
    num_atoms = atoms.size();
...
kappa::Mixture::Mixture(const kappa::Molecule &molecule, const kappa::Atom &atom, const std::string &interactions_filename, const std::string &particles_filename):kappa::Approximation() {
    // cannot have ionized!
...
kappa::Mixture::Mixture(const kappa::Molecule &molecule, const std::string &interactions_filename, const std::string &particles_filename):kappa::Approximation() {
...
kappa::Mixture::Mixture(const kappa::Atom &atom, const std::string &interactions_filename, const std::string &particles_filename):kappa::Approximation() {
...
kappa::Mixture::Mixture(const std::string particle_names, const std::string &interactions_filename, const std::string &particles_filename, bool anharmonic, bool rigid_rotators):kappa::Approximation() {
...
void kappa::Mixture::init_matrices(const std::string &particles_filename) {
    ...
    shear_viscosity_RHS.zeros(n_particles);
...
int kappa::Mixture::inter_index(int i, int j) { // return interaction index for particles with indices i and j
	if (i <= j) {
...
double kappa::Mixture::debye_length(double T, const arma::vec &n_molecule, const arma::vec &n_atom, double n_electrons) {
    ...
    return sqrt(K_CONST_E0 * K_CONST_K * T / (arma::dot(n_molecule, molecule_charges_sq)
...
double kappa::Mixture::debye_length(double T, const std::vector<arma::vec> &n_vl_molecule, double n_electrons) {
    ...
    return sqrt(K_CONST_E0 * K_CONST_K * T / (arma::dot(n_mol, molecule_charges_sq)
                                              + n_electrons * K_CONST_ELEMENTARY_CHARGE * K_CONST_ELEMENTARY_CHARGE));
}
...
// arma::vec kappa::Mixture::compute_density_molecule(const std::vector<arma::vec> &n_vl_molecule) {
//     #ifdef KAPPA_STRICT_CHECKS
//     std::string error_string;
//     if (n_vl_molecule.size() != num_molecules) {
//         error_string = "Incorrect size of array of vibrational level populations of molecules, wrong number of molecules: ";
...
//     }
//     int ii;
//     for (ii = 0; ii < num_molecules; ii++) {
//         if (molecules[ii].num_vibr_levels[0] != n_vl_molecule[ii].n_elem) {
//             error_string = "Incorrect size of array of vibrational level populations of molecule " + molecules[ii].name + ": ";
//             error_string += "passed array is of size " + std::to_string(n_vl_molecule[ii].n_elem) + ", should be of size " + std::to_string(molecules[ii].num_vibr_levels[0]);
//             throw kappa::IncorrectValueException(error_string.c_str());
//         }
//         for (int kk = 0; kk < molecules[ii].num_vibr_levels[0]; kk++) {
//             if (n_vl_molecule[ii][kk] < 0) {
//                 error_string = "Array of vibrational level populations of molecule " + molecules[ii].name + " contains at least one negative value: ";
//                 error_string += "n_vl_molecule[" + std::to_string(ii) + "][" + std::to_string(kk) + "]=" + std::to_string(n_vl_molecule[ii][kk]);
//                 throw kappa::IncorrectValueException(error_string.c_str());
//             }
//         }
//     }
...
const arma::mat &kappa::Mixture::compute_bulk_viscosity_LHS(double T, kappa::models_omega model) {
    ...
    for (i = 0; i < num_molecules; i++) { // molecule + molecule collisions for molecules of different species or at different vibrational levels
        for (j = i; j < num_molecules; j++) {
                ...
                for (l=0; l < molecules[j].num_vibr_levels[0]; l++) {
                    ...
                    if ((j!=i) || (l>k)) {
                        ...
                        bulk_viscosity_LHS.at(p1, p2 + 1) = -4 * T * n_ij * molecules[i].mass * molecules[i].mass * c_rot_arr[o1]
                                                               / ((molecules[i].mass + molecules[j].mass) * K_CONST_PI * eta_cd * rm * rot_rel_times.at(i,j));
    ...
    for (i = 0; i < num_molecules; i++) { // molecule + molecule collisions for identical molecular species at identical vibrational levels
        ...
        for (k = 0; k < molecules[i].num_vibr_levels[0]; k++) {
            ...
            bulk_viscosity_LHS.at(p1 + 1, p1) = -tmp; // 1001

            bulk_viscosity_LHS.at(p1, p1 + 1) = -tmp; // 0110
        ...
        for (j = 0; j < num_atoms; j++) {
            ...
            for (k = 0; k < molecules[i].num_vibr_levels[0]; k++) {
                ...
                bulk_viscosity_LHS.at(p1, p1) += 5 * kT * n_ij * coll_mass / ((molecules[i].mass + atoms[j].mass) * A_cd * eta_cd) +  4 * T * n_ij * atoms[j].mass * atoms[j].mass
    ...
    for (i = 0; i < num_atoms; i++) { // atom + atom collisions for identical atomic species
        ...
        for (j = 0; j < num_molecules; j++) {
            ...
            for (k = 0; k < molecules[j].num_vibr_levels[0]; k++) {
                ...
                bulk_viscosity_LHS.at(n_vibr_levels_total * 2 + i, n_vibr_levels_total * 2 + i) += 5 * kT * n_ij * coll_mass / ((atoms[i].mass + molecules[j].mass) * A_cd * eta_cd) +  4 * T * n_ij * molecules[j].mass * molecules[j].mass
...
const arma::vec &kappa::Mixture::compute_bulk_viscosity_RHS(double T) {
    ...
    for (i = 0; i < num_molecules; i++) {
        for (k = 0; k < molecules[i].num_vibr_levels[0]; k++) {
            bulk_viscosity_RHS[2 * j] = -this_n_vl_mol[i][k] * this_crot / this_total_n;
            bulk_viscosity_RHS[2 * j + 1] = this_n_vl_mol[i][k] * molecules[i].mass * c_rot_arr[j] / this_total_dens; // TODO: vectorize!
            j++;
        }
    }
    for (i = 0; i < num_atoms; i++) {
    ...
    bulk_viscosity_RHS[0] = 0; // normalization condition
...
const arma::mat &kappa::Mixture::compute_thermal_conductivity_LHS(double T, kappa::models_omega model) {
            ...
            for (k = 0; k < molecules[i].num_vibr_levels[0]; k++) {
                ...
                for (l=0; l < molecules[j].num_vibr_levels[0]; l++) {
                    ...
                    if ((j!=i) || (l>k)) {
                        ...
                        thermal_conductivity_LHS.at(p1, p2 + 1) = 0.75 * kT * n_ij * (6 * C_cd - 5) * molecules[i].mass / ((molecules[i].mass + molecules[j].mass) * n * D_cd);
                        thermal_conductivity_LHS.at(p1 + 1, p2 + 1) = -1.5 * kT * n_ij * coll_mass / (n * D_cd * (molecules[i].mass + molecules[j].mass))
                        ...
                        thermal_conductivity_LHS.at(p1 + 2, p2 + 1) = -6 * T / (n * D_cd * K_CONST_PI) * A_cd * n_ij * molecules[i].mass * molecules[i].mass
                                                                                 * c_rot_arr[o1] / (rm * rot_rel_times.at(i, j) * (molecules[i].mass + molecules[j].mass)); // 0110

                        thermal_conductivity_LHS.at(p1, p2 + 2) = 0; // 0001
                        thermal_conductivity_LHS.at(p1 + 1, p2 + 2) = -6 * T / (n * D_cd * K_CONST_PI) * A_cd * n_ij * molecules[j].mass * molecules[j].mass
                                                                                 * c_rot_arr[o2] / (rm * rot_rel_times.at(j, i) * (molecules[i].mass + molecules[j].mass)); // 1001
    ...
    for (i = 0; i < num_molecules; i++) { // molecule + molecule collisions for identical molecular species at identical vibrational levels
        ...
        for (j = 0; j < num_atoms; j++) {
            ...
            for (k = 0; k < molecules[i].num_vibr_levels[0]; k++) {
                ...
                thermal_conductivity_LHS.at(p1 + 1, p1 + 1) += 1.5 * kT * coll_mass * n_ij * (7.5 * molecules[i].mass / atoms[j].mass
                ...
                thermal_conductivity_LHS.at(p1 + 1, p1 + 2) -= 6. * T * A_cd * n_ij * molecules[i].mass * c_rot_arr[o1] / (rm * rot_rel_times.at(i, num_molecules + j)) * molecules[i].mass
                                                                               / (n * D_cd * K_CONST_PI * (molecules[i].mass + atoms[j].mass)); // 0110
                thermal_conductivity_LHS.at(p1 + 2, p1 + 2) += T * n_ij * molecules[i].mass * c_rot_arr[o1] * (1.5 / D_cd_rot + 3.6 * A_cd * molecules[i].mass / (K_CONST_PI * D_cd * atoms[j].mass
    ...
    for (i = 0; i < num_atoms-1; i++) { // atoms + atom collisions for different atomic species
        for (j = i+1; j < num_atoms; j++) {
            ...
            thermal_conductivity_LHS.at(n_vibr_levels_total * 3 + 2 * i + 1, n_vibr_levels_total * 3 + 2 * j) = 0.75 * kT * n_ij * (6 * C_cd - 5) * atoms[j].mass / ((atoms[i].mass + atoms[j].mass) * n * D_cd); // 1000
    ...
    for (i = 0; i < num_atoms; i++) { // atom + atom collisions for identical atomic species
        ...
        for (j = 0; j < num_molecules; j++) {
            ...
            for (k = 0; k < molecules[j].num_vibr_levels[0]; k++) {
                ...
                thermal_conductivity_LHS.at(n_vibr_levels_total * 3 + 2 * i + 1, n_vibr_levels_total * 3 + 2 * i + 1) += 1.5 * kT * coll_mass * n_ij * (7.5 * atoms[i].mass / molecules[j].mass
                                                                                                           + 6.25 * molecules[j].mass / atoms[i].mass - 3 * molecules[j].mass * B_cd / atoms[i].mass
                                                                                                           + 4 * A_cd + (20./3) * A_cd * (molecules[j].mass * c_rot_arr[o1] / (rm * rot_rel_times.at(num_molecules + j, i)))
        ...
        for (j = 0; j < num_atoms; j++) {
            if (j!=i) {
                ...
                thermal_conductivity_LHS.at(n_vibr_levels_total * 3 + 2 * i + 1, n_vibr_levels_total * 3 + 2 * i + 1) += 1.5 * kT * coll_mass * n_ij * (7.5 * atoms[i].mass / atoms[j].mass
...
double kappa::Mixture::thermal_conductivity(double T, kappa::models_omega model) {
    ...
    if (all_rigid_rotators) {
        ...
        for (i = 0; i < num_molecules; i++) {
            res += this_n_molecules[i] * (1.25 * K_CONST_K * thermal_conductivity_rigid_rot_coeffs[3 * i + 1] + 0.5 * thermal_conductivity_rigid_rot_coeffs[3 * i + 2] * c_rot_rigid_rot_arr[i] * molecules[i].mass);
		}
        ...
        for (i = 0; i < num_molecules; i++) {
            for (k = 0; k < molecules[i].num_vibr_levels[0]; k++) {
                res += this_n_vl_mol[i][k] * (1.25 * K_CONST_K * thermal_conductivity_coeffs[3*j + 1] + 0.5 * thermal_conductivity_coeffs[3*j + 2] * c_rot_arr[j] * molecules[i].mass);
				j++;
            } // TODO: vectorize???
        }
...
const arma::mat &kappa::Mixture::compute_diffusion_LHS(double T) {
    ...
    for (i=0; i < num_atoms; i++) {
        for (k=0; k < num_atoms; k++) {
            diffusion_LHS(n_vibr_levels_total + i,
                          n_vibr_levels_total + k) = thermal_conductivity_LHS.at(n_vibr_levels_total * 3 + i * 2,
...
const arma::vec &kappa::Mixture::compute_diffusion_RHS(double T, int b, int n) {
    ...
    from f-la (5.17), for fixed (c,i,b,n) we have
    ...
    */
    int i, k, offset=0;
    for (i=0; i < num_molecules; i++) { // over molecules
        for (k=0; k < molecules[i].num_vibr_levels[0]; k++) { // over vibr. levels
            if ((i==b) && (k==n)) {
...
void kappa::Mixture::diffusion(double T) {
    ...
    for (i=0; i < num_molecules; i++) { // over molecules
        for (k=0; k < molecules[i].num_vibr_levels[0]; k++) { // over vibr. levels
            ...
            // diff(offset) = compute_diffusion_coeffs(T, i, k)
            offset += 1;
        }
    }
    for (i=0; i<num_atoms; i++) {
...
const arma::mat &kappa::Mixture::compute_shear_viscosity_LHS(double T, kappa::models_omega model) {
    ...
    for (i1 = 0; i1 < num_molecules - 1; i1++) { // molecule + molecule collisions for different molecular species
        for (i2 = i1+1; i2 < num_molecules; i2++) {
                tmp = 3.2 * (this_n_molecules[i1] / n) * (this_n_molecules[i2] / n) * (molecules[i1].mass * molecules[i2].mass)
                          * omega_22.at(i1, i2) / (kT * (molecules[i1].mass + molecules[i2].mass) * (molecules[i1].mass + molecules[i2].mass)); // 3.2 = 16/5
    ...
    for (i1 = 0; i1 < num_molecules; i1++) { // molecule + molecule collisions for identical species
        ...
        for (i2 = 0; i2 < num_molecules; i2++) {
            if (i2 != i1) {
                tmp += (3.2 * (this_n_molecules[i1] / n) * (this_n_molecules[i2] / n) * omega_22.at(i1, i2) / kT)
                    *  (10. * omega_11.at(i1, i2) * molecules[i2].mass * molecules[i1].mass / ((molecules[i2].mass + molecules[i1].mass) * (molecules[i2].mass + molecules[i1].mass) * 3. * omega_22.at(i1, i2))
                        + molecules[i2].mass * molecules[i2].mass / ((molecules[i2].mass + molecules[i1].mass) * (molecules[i2].mass + molecules[i1].mass)));
            }        
        }

        for (i2 = 0; i2 < num_atoms; i2++) { // loop over all atomic species
    ...
    for (i1 = 0; i1 < num_molecules; i1++) { // molecule + atom collisions
        for (i2 = 0; i2 < num_atoms; i2++) { // loop over all atoms
    ...
    for (i1 = 0; i1 < num_atoms - 1; i1++) { // atom + atom collisions for different atomic species
    ...
    for (i1 = 0; i1 < num_atoms; i1++) { // atom + atom collisions for identical species
        tmp = 1.6 * (this_n_atom[i1] / n) * (this_n_atom[i1] / n) * omega_22.at(num_molecules + i1, num_molecules + i1) / kT;  // 1.6 = 8/5

        for (i2 = 0; i2 < num_molecules; i2++) { // loop over all molecular species
