...
int main(int argc, char** argv) {
  ...
  //mol_ndens.push_back(mixture.Boltzmann_distribution(T_vals[0], x_O2 * pressure / (K_CONST_K * T_vals[0]), mol)); // O2

  std::ofstream outf;
  outf.open(output_dir + "/TRANSPORT_COEFFICIENTS/shear_viscosity/" + mol.name + "_xat_" + std::to_string(x_N2) + ".txt");
  //outf.open(output_dir + "/TRANSPORT_COEFFICIENTS/shear_viscosity/" + mol.name + "_xat_" + std::to_string(x_O2) + ".txt");
...
//                                                     models_omega::model_omega_esa};
  ...
  for (auto T : T_vals) {
    ...
    //mol_ndens[0] = mixture.Boltzmann_distribution(T, x_O2 * pressure / (K_CONST_K * T), mol);
    //atom_ndens[0] = x_O * pressure / (K_CONST_K * T);
